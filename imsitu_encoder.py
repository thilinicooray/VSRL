...
class imsitu_encoder():
    def __init__(self, train_set):
        ...
        for img_id in train_set:
            ...
            if current_verb not in self.verb_list:
                ...
                verb2_role_dict[current_verb] = []

            for frame in img['frames']:
                for role,label in frame.items():
                    if role not in self.role_list:
                        self.role_list.append(role)
                    if role not in verb2_role_dict[current_verb]:
                        verb2_role_dict[current_verb].append(role)
                    if len(verb2_role_dict[current_verb]) > self.max_role_count:
                        self.max_role_count = len(verb2_role_dict[current_verb])
        ...
        self.verb_embedding = torch.eye(len(self.verb_list)).view(len(self.verb_list),len(self.verb_list),1)
        self.role_embedding = torch.eye(len(self.role_list)).view(len(self.role_list),len(self.role_list),1)
        self.label_embedding = torch.eye(len(self.label_list)).view(len(self.label_list),len(self.label_list),1)
        ...
        for verb_id in range(len(self.verb_list)):
            current_role_list = verb2_role_dict[self.verb_list[verb_id]]
            ...
            for i in range(padding_count):
                role_embedding_verb.append(torch.zeros(len(self.role_list),1))
    ...
    def encode(self, item):
        #todo:handle never before seen words -> map them to UNK
        ...
        for frame in item['frames']:
            ...
            for i in range(role_padding_count):
                label_embedding_list.append(torch.zeros(len(self.label_list),1))
